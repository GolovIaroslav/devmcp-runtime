...
package-smoke:
	...
	test -n "$$wheel"; \
	uv pip install --python "$$tmp_dir/venv/bin/python" --no-deps "$$wheel"; \
	"$$tmp_dir/venv/bin/devmcp" --version; \
	"$$tmp_dir/venv/bin/python" -c 'import coding_tools_mcp; assert coding_tools_mcp.__version__ == "0.1.0-beta.1"'
