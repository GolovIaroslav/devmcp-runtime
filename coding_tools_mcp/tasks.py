...
TASK_REGISTRY = [
    ...
    _task("test.dummy", "test", "Safe dummy test task.", "python3", ["-c", "print('dummy_task_ok')"]),
    _task("project.detect", "project", "Detect project type.", "python3", ["-c", "import os; print('package.json' if os.path.exists('package.json') else 'pyproject.toml')"]),
    ...
    _task("json.validate", "json", "Validate JSON syntax.", "python3", ["-m", "json.tool"]),
    _task("yaml.validate", "yaml", "Validate YAML syntax.", "python3", ["-c", "import yaml, sys; yaml.safe_load(open(sys.argv[1]))"]),
    _task("toml.validate", "toml", "Validate TOML syntax.", "python3", ["-c", "import tomllib, sys; tomllib.load(open(sys.argv[1], 'rb'))"]),
    _task("git.status", "git", "Show git status.", "git", ["status"]),
    _task("git.diff", "git", "Show git diff.", "git", ["diff"]),
    _task("git.diff_cached", "git", "Show staged git diff.", "git", ["diff", "--cached"]),
    ...
    _task("git.current_branch", "git", "Show current git branch.", "git", ["branch", "--show-current"]),
    ...
    _task("npm.format_check", "npm", "Run npm format check.", "npm", ["run", "format:check"]),
    _task("npm.install", "npm", "Install npm dependencies with an approved network grant.", "npm", ["install"], network=True, approval="ASK"),
    ...
    _task("pnpm.install", "pnpm", "Install pnpm dependencies with an approved network grant.", "pnpm", ["install"], network=True, approval="ASK"),
    ...
    _task("yarn.install", "yarn", "Install yarn dependencies with an approved network grant.", "yarn", ["install"], network=True, approval="ASK"),
    ...
    _task("ruff.format_check", "python", "Run ruff format check.", "ruff", ["format", "--check", "."]),
    _task("mypy.check", "python", "Run mypy type checking.", "mypy", ["."]),
    _task("pyright.check", "python", "Run pyright.", "pyright"),
    _task("python.compileall", "python", "Compile all Python files.", "python3", ["-m", "compileall", "."]),
    _task("python.build", "python", "Build the Python package.", "python3", ["-m", "build"]),
    _task("tox.run", "python", "Run tox.", "tox"),
    _task("nox.run", "python", "Run nox.", "nox"),
    _task("uv.sync", "python", "Sync uv dependencies with an approved network grant.", "uv", ["sync"], network=True, approval="ASK"),
    ...
    _task("cargo.fmt_check", "rust", "Run cargo fmt check.", "cargo", ["fmt", "--", "--check"]),
    ...
    _task("go.mod_tidy", "go", "Tidy Go dependencies with an approved network grant.", "go", ["mod", "tidy"], network=True, approval="ASK"),
    ...
    _task("cmake.configure", "c", "Configure CMake.", "cmake", ["-S", ".", "-B", "build"]),
    _task("cmake.build", "c", "Build CMake project.", "cmake", ["--build", "build"]),
    _task("cmake.test", "c", "Run CTest.", "ctest", ["--test-dir", "build"]),
    _task("playwright.test", "browser", "Run Playwright tests.", "npx", ["playwright", "test"]),
    _task("playwright.test_target", "browser", "Run a Playwright target.", "npx", ["playwright", "test"]),
    _task("playwright.smoke", "browser", "Run Playwright smoke tests.", "npx", ["playwright", "test", "--grep", "@smoke"]),
    _task("cypress.run", "browser", "Run Cypress.", "npx", ["cypress", "run"]),
    _task("prisma.generate", "db", "Generate Prisma client.", "npx", ["prisma", "generate"]),
    _task("prisma.db_push", "db", "Push Prisma schema.", "npx", ["prisma", "db", "push"], approval="ASK"),
    _task("alembic.upgrade", "db", "Upgrade the Alembic database.", "alembic", ["upgrade", "head"], approval="ASK"),
    _task("http.health", "network", "Check HTTP health with an approved network grant.", "curl", ["-sI"], network=True, approval="ASK"),
]
...
class TaskRegistry:
    ...
    def resolve_command(self, task_id: str, args: list[str] | str | None = None, path: str | None = None) -> list[str]:
        task = self.tasks.get(task_id)
        if task is None:
            raise ToolFailure("NOT_FOUND", f"Task {task_id} not found.", category="validation")
    ...
    def build_argv(self, template: TaskTemplate, args_input: dict[str, Any] | None = None) -> list[str]:
        values = args_input or {}
        typed: dict[str, Any] = {}
        for name, definition in template.arg_definitions.items():
            if name not in values or values[name] is None:
                if definition.get("required"):
                    raise ToolFailure("INVALID_ARGUMENT", f"Task argument '{name}' is required.", category="validation")
                continue
        ...
        if not argv or any(not isinstance(item, str) or not item for item in argv):
            raise ToolFailure("INVALID_ARGUMENT", "Task template produced an invalid argv.", category="validation")
        return argv
    ...
    def _validate_argument(name: str, value: Any, definition: dict[str, Any]) -> Any:
        ...
        if type_name == "string":
            ...
            valid = isinstance(value, (list, tuple)) and all(isinstance(item, str) and item for item in value)
            ...
            if valid:
                ...
                valid = not pure.is_absolute() and ".." not in pure.parts and "" not in pure.parts
        elif type_name == "enum":
