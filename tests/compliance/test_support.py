...
class ComplianceTestCase(unittest.TestCase):
    ...
    @contextmanager
    def client_with_permission(self, permission_mode: str) -> Iterator[MCPClient]:
        with MCPClient(self.workspace.root, permission_mode=permission_mode) as client:
            yield client

    def assert_tool_success(self, result: dict[str, Any]) -> dict[str, Any]:
        self.assertFalse(
            result.get("isError", False), f"expected tool success, got {result!r}"
        )
        self.assertIn(
            "content", result, f"MCP tool result must contain content: {result!r}"
        )
        self.assertIsInstance(
            result["content"], list, f"content must be a list: {result!r}"
        )
    ...
    def assert_tool_error(
        ...
        try:
            ...
            self.assertIn(
                "code", exc.error, f"JSON-RPC error must include code: {exc.error!r}"
            )
            self.assertIn(
                "message",
                exc.error,
                f"JSON-RPC error must include message: {exc.error!r}",
            )
            return {"rpc_error": exc.error}
        self.assertTrue(
            result.get("isError", False),
            f"expected tool error for {tool_name}, got {result!r}",
        )
        payload = structured_payload(result)
        self.assertTrue(
            payload or result.get("content"),
            f"tool error must be structured or contain content: {result!r}",
        )
        return payload

    def assert_denied_or_permission_required(
        self, tool_name: str, arguments: dict[str, Any]
    ) -> dict[str, Any]:
