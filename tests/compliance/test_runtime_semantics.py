...
class RuntimeSemanticsTests(ComplianceTestCase):
                ...
                with workspace_from_fixture("tiny-js-project") as workspace:
                    with MCPClient(workspace.root, permission_mode="trusted") as client:
                        ...
                        if case["expect_success"]:
                            ...
                            self.assertTrue(
                                result.get("isError"),
                                f"case should fail: {case!r} -> {result!r}",
                            )
    ...
    def test_session_semantics_match_runtime_exec_and_stdin(self) -> None:
        with self.session_for_fixture("long-running-project") as (_workspace, client):
            ...
            output = client.call_tool(
                "write_stdin", {"session_id": session_id, "chars": "compat\n"}
            )
            pass
    ...
    def test_missing_and_closed_sessions_return_structured_errors(self) -> None:
        self.assert_denied_or_permission_required(
            "write_stdin", {"session_id": "missing-session", "chars": "hello\n"}
        )
        with self.session_for_fixture("long-running-project") as (_workspace, client):
            started = client.call_tool(
                "exec_command",
            ...
            try:
                closed = client.call_tool(
                    "write_stdin", {"session_id": session_id, "chars": "after-close\n"}
                )
            except MCPError:
                return
            self.assertTrue(
                closed.get("isError"), f"write to closed session must fail: {closed!r}"
            )
    ...
    def test_view_image_semantics_when_p1_tool_is_present(self) -> None:
        with self.session_for_fixture("image-project") as (_workspace, client):
            ...
            self.assertRegex(
                self.tool_text(image), r"(data:image/png;base64|image/png)"
            )
            try:
                denied = client.call_tool(
                    "view_image", {"path": "../outside-secret.txt"}
                )
            except MCPError:
                return
            self.assertTrue(
                denied.get("isError"), f"view_image traversal must fail: {denied!r}"
            )
